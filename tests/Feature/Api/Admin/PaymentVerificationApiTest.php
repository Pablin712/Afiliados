...
class PaymentVerificationApiTest extends TestCase
{
    ...
    public function test_approving_an_affiliate_payment_grants_direct_sale_commission_without_touching_sponsor_rank(): void
    {
        ...
        // Sponsor already holds a legacy rank (constructor); the new logic must not
        // recalculate or change it anymore on payment approval.
        ...
        // A prior approved sale this year gives the sponsor 100 points before the next one.
        $priorBuyer = User::factory()->create(['sponsor_id' => $sponsor->id]);
        Payment::query()->create([
            'user_id' => $priorBuyer->id,
            'number' => 'CMP-9000',
            'amount' => 150,
            'state' => 'approved',
            'reviewed_at' => now(),
        ...
        $transaction = Transaction::query()->create([
            ...
            'amount' => 0,
            'amount_now' => 0,
            'detail' => 'Pending direct sale test transaction',
            'is_annulled' => false,
            'created_at' => now(),
        ]);
        ...
        $response = $this->withHeaders([
            ...
            'trace_id' => 'n8n-trace-direct-sale',
        ]);

        $response->assertOk();

        // Rank stays exactly as it was: no more auto-recalculation on approval.
        $this->assertDatabaseHas('memberships', [
            'user_id' => $sponsor->id,
            'membership_type_id' => $constructorType->id,
            'status' => 'active',
        ...
        $this->assertFalse(
            Profit::query()
                ...
                ->where('detail', 'like', 'rank_bonus|%')
                ->exists()
        );

        $sponsor->refresh();

        $this->assertSame(30.0, (float) $sponsor->commission_balance);
    }
