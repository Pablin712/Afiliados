                        ...
                        @elseif (!$pendingPayment)
                            ...
                            <div
                                ...
                                x-data="{
                                    ...
                                    async startPayphone() {
                                        this.ppState = 'loading';
                                        this.ppError = null;
                                        try {
                                            const resp = await fetch(@js(route('plans.payphone.prepare')), {
                                                method: 'POST',
                                                headers: {
                                                ...
                                                body: JSON.stringify({ program_id: @js($program->id) }),
                                            });
                                            const data = await resp.json();
                                            if (!resp.ok) {
                                                this.ppError = data.message || @js(__('messages.plans.payphone_prepare_error'));
                                                this.ppState = 'error';
                                                return;
                                            }
                                            this.ppPaymentId = data.payment_id;
                                            this.ppState = 'ready';
                                            this.$nextTick(() => {
                                                new PPaymentButtonBox({
                                                    token: this.ppPaymentId,
                                                    btnHorizontal: true,
                                                    btnClose: true,
                                                }).render('payphone-widget-{{ $program->id }}');
                                            });
                                        } catch {
                                            this.ppError = @js(__('messages.plans.payphone_prepare_error'));
                                            this.ppState = 'error';
                                        }
                                    }
                                }"
                            >
                                ...
                                <div class="flex gap-2 mb-5">
                                    <button
                                        type="button"
                                        @click="paymentTab = 'bank'; ppState = 'idle'; ppError = null; ppPaymentId = null"
                                        :class="paymentTab === 'bank'
                                            ? 'bg-brand-600 text-white'
                                            : 'bg-gray-100 text-gray-700 hover:bg-gray-200 dark:bg-graphite-800 dark:text-graphite-300 dark:hover:bg-graphite-700'"
                                    ...
                                    <button
                                        type="button"
                                        @click="paymentTab = 'payphone'"
                                        :class="paymentTab === 'payphone'
                                            ? 'bg-brand-600 text-white'
                                            : 'bg-gray-100 text-gray-700 hover:bg-gray-200 dark:bg-graphite-800 dark:text-graphite-300 dark:hover:bg-graphite-700'"
                                        class="rounded-lg px-4 py-2 text-sm font-medium transition-colors"
                                    >
                                ...
                                {{-- Bank transfer form --}}
                                <div x-show="paymentTab === 'bank'" x-cloak>
                                    @if ($banks->isEmpty())
                                ...
                                {{-- Payphone card payment --}}
                                <div x-show="paymentTab === 'payphone'" x-cloak>
                                    <h4 class="text-base font-semibold text-gray-900 dark:text-graphite-100">
                                        {{ __('messages.plans.payphone_title') }}
                                    </h4>
                                    <p class="mt-1 text-sm text-gray-600 dark:text-graphite-400">
                                        {{ __('messages.plans.payphone_description') }}
                                    </p>

                                    <div class="mt-4 flex items-center justify-between rounded-lg border border-brand-200 bg-brand-50 px-4 py-3 dark:border-brand-900/60 dark:bg-brand-900/20">
                                        <span class="text-sm text-gray-700 dark:text-graphite-300">{{ __('messages.plans.fixed_amount_note') }}</span>
                                        <span class="text-lg font-bold text-brand-700 dark:text-brand-300">${{ number_format($fixedAmount, 2) }}</span>
                                    </div>

                                    <div class="mt-5">
                                        <div x-show="ppState === 'idle'">
                                            <button
                                                type="button"
                                                @click="startPayphone"
                                                class="inline-flex items-center gap-2 rounded-lg bg-brand-600 px-5 py-2.5 text-sm font-semibold text-white shadow-sm hover:bg-brand-700 focus:outline-none focus:ring-2 focus:ring-brand-500 focus:ring-offset-2 transition-colors"
                                            >
                                                ...
                                                {{ __('messages.plans.payphone_start_button') }}
                                            </button>
                                        </div>

                                        <div x-show="ppState === 'loading'" class="flex items-center gap-2 text-sm text-gray-600 dark:text-graphite-400">
                                            ...
                                            {{ __('messages.plans.payphone_loading') }}
                                        </div>

                                        <div x-show="ppState === 'error'" class="rounded-md border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-800 dark:border-red-700 dark:bg-red-900/20 dark:text-red-300">
                                            <p x-text="ppError"></p>
                                            <button type="button" @click="ppState = 'idle'; ppError = null" class="mt-2 text-xs underline">
                                                {{ __('messages.plans.payphone_retry') }}
                                            </button>
                                        </div>

                                        <div x-show="ppState === 'ready'" id="payphone-widget-{{ $program->id }}" class="mt-4"></div>
                                    </div>
                                </div>
...
@push('scripts')
<script src="https://cdn.payphonetodoesposible.com/app/button/1.0.0/payphone-payment-button.js" defer></script>
@endpush
</x-app-layout>
