    ...
    <x-modal name="testimonial-edit-modal" focusable>
        <form id="testimonial-edit-form" method="POST" action="{{ route('admin.testimonials.update', ['testimonial' => '__ID__']) }}" class="p-6 space-y-4" enctype="multipart/form-data">
            ...
            <div>
                ...
                <div class="flex items-center gap-4">
                    <label class="inline-flex items-center gap-2 text-sm text-gray-700 dark:text-graphite-300">
                        <input id="testimonial-edit-media-type-image" type="radio" name="media_type" value="image" onchange="window.toggleTestimonialMediaFields(this.form, 'image')" class="border-gray-300 dark:border-graphite-700">
                        {{ __('messages.admin.testimonials.forms.media_type_image') }}
                    </label>
                    <label class="inline-flex items-center gap-2 text-sm text-gray-700 dark:text-graphite-300">
                        <input id="testimonial-edit-media-type-video" type="radio" name="media_type" value="video" onchange="window.toggleTestimonialMediaFields(this.form, 'video')" class="border-gray-300 dark:border-graphite-700">
            ...
            <div data-media-field="video" class="hidden">
                ...
                <video id="testimonial-edit-current-video" src="" controls class="h-24 w-16 rounded-md border border-gray-200 dark:border-graphite-700 hidden mb-2"></video>
                <input id="testimonial-edit-video" type="file" name="video" accept="video/mp4,video/quicktime,video/webm" class="w-full text-sm text-gray-700 dark:text-graphite-200">
                <p class="mt-1 text-xs text-gray-400">{{ __('messages.admin.testimonials.forms.video_hint_edit') }}</p>
                @error('video')
                    <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                @enderror
            </div>
