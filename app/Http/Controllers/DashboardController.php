...
class DashboardController extends Controller
{
    ...
    public function __invoke(Request $request): View
    {
        ...
        return view('dashboard', [
            'user' => $user,
            'currentMembership' => $user->membership?->membershipType?->name ?? 'Free',
            ...
            'monthlyProfitsAmount' => (float) (clone $monthlyProfitsQuery)->sum('amount'),
            'monthlyApprovedPaymentsCount' => (int) ($hasSourcePaymentColumn
                ? (clone $monthlyProfitsQuery)->whereNotNull('source_payment_id')->count()
                : (clone $monthlyProfitsQuery)->count()),
            'pendingProfitsAmount' => (float) (clone $pendingProfitsQuery)->sum('amount'),
            'paidProfitsAmount' => (float) (clone $paidProfitsQuery)->sum('amount'),
            'recentProfits' => Profit::query()
